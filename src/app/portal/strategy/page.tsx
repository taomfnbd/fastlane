...
export default async function PortalStrategyPage() {
  ...
  const pending = strategies.filter((s) => needsReview(s.status) || s.status === "CHANGES_REQUESTED");
  const done = strategies.filter((s) => s.status === "APPROVED");
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-2 text-[11px] text-muted-foreground">
                          <span>{strategy.eventCompany.event.name}</span>
                          <span>v{strategy.version}</span>
                          {totalItems > 0 && <span>{approvedItems}/{totalItems}</span>}
                          {strategy._count.comments > 0 && (
                            <span className="inline-flex items-center gap-1">
                              <MessageSquare className="h-3 w-3" />
                              {strategy._count.comments}
