...
interface DeliverableReviewActionsProps { deliverableId: string; }
...
export function DeliverableReviewActions({ deliverableId }: DeliverableReviewActionsProps) {
  ...
  async function handleApprove() {
    ...
    if (result.success) { toast.success("Livrable approuve"); }
  ...
  async function handleRequestChanges() {
    ...
    if (result.success) { toast.success("Modifications demandees"); }
    else { toast.error(result.error); }
    setLoading(false);
  }
  ...
  return (
    <div className="flex gap-2">
      ...
      <Button
        ...
        onClick={handleRequestChanges} disabled={loading}
      >
